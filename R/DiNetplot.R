#' DiNetplot
#'
#' This function display network plot using Shiny and visNetwork packages.
#' @param TCGANetwork_list igraph list generated by TCGANetwork().
#' @examples
...
DiNetplot <- function(TCGANetwork_list){
  ...
  server <- function(input, output) {
    ...
    output$vis1 <- visNetwork::renderVisNetwork(
      {
        visNetwork::visNetwork(nodes1, edges1, width = "100%") %>%
    ...
    output$vis2 <- visNetwork::renderVisNetwork(
      {
        visNetwork::visNetwork(nodes2, edges2, width = "100%") %>%
          visNetwork::visIgraphLayout() %>%
          visNetwork::visNodes(
            shape = "dot",
            color = list(
